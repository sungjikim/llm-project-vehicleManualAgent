...
- 📈 **77.8% 성능 향상**: 응급 질문 응답시간 11.5초 → 2.55초
- 🔄 **조건부 워크플로우**: 상황에 따라 적절한 처리 경로 자동 선택
- ⚡ **간소화된 검색**: 응급 상황에서 BM25 키워드 검색만 사용
...
| 질문 유형 | 기존 응답시간 | 개선된 응답시간 | 성능 향상 |
|-----------|---------------|-----------------|-----------|
| **응급 질문** | 11.5초 | 2.55초 | **77.8% 향상** |
| **일반 질문** | 8.2초 | 8.2초 | 유지 |
| **주행 중 질문** | 8.2초 | 6.1초 | **25.6% 향상** |
...
### 📋 **지능형 검색기 선택 시스템 (실제 구현 기반)**

시스템은 **LLM 분석 + 키워드 규칙 하이브리드** 방식으로 질문의 특성을 분석하여 최적의 검색 방법을 자동 선택합니다.

#### 🧠 **1단계: LLM 기반 쿼리 분석**

LLM이 Few-shot 예시를 통해 질문을 분석하여 초기 검색 전략과 방법을 결정합니다.

| LLM 분석 결과 | 전략 | 방법 | 가중치 | 설명 |
|---------------|------|------|--------|------|
| **General** | 일반 정보 | `hybrid_semantic` | 7:3 | 의미론적 검색 우선 |
| **Specific** | 구체적 사양 | `hybrid_balanced` | 5:5 | 균형 잡힌 검색 |
| **Troubleshooting** | 문제 해결 | `hybrid_keyword` | 3:7 | 키워드 검색 우선 |

#### ⚡ **2단계: 키워드 규칙 오버라이드**

**키워드 규칙이 LLM 분석 결과보다 우선 적용**됩니다.

| 키워드 조건 | 강제 선택 | 검색 방식 | 예시 질문 |
|-------------|-----------|-----------|-----------|
| `'교체', '문제', '고장', '이상'` | **`expanded_query`** | 전문 용어 확장 + 하이브리드 | "브레이크 문제" → "브레이크 문제 브레이크 제동 제동장치..." |
| `len(query) > 20 and '?' in query` | **`multi_query`** | 3개 관점 쿼리 생성 + 벡터 | "XC60의 자동 주차 기능은 어떻게 작동하고, 언제 사용할 수 있나요?" |
| 일반 질문 | **LLM 분석 결과 사용** | LLM이 선택한 방법 | "타이어 공기압은 얼마인가요?" |

#### 🔍 **최종 검색 방법별 특징**

| 방법 | 트리거 조건 | 검색 방식 | 처리 시간 | 정확도 |
|------|-------------|-----------|-----------|--------|
| **expanded_query** | 교체/문제/고장/이상 키워드 | 전문 용어 확장 + 하이브리드 | 빠름 | 중간 |
| **multi_query** | 복잡한 질문 (20자+ + '?') | 3개 관점 쿼리 생성 + 벡터 | 느림 | 높음 |
| **hybrid_semantic** | LLM: general | 의미론적 우선 (7:3) | 중간 | 높음 |
| **hybrid_balanced** | LLM: specific | 균형 검색 (5:5) | 중간 | 높음 |
| **hybrid_keyword** | LLM: troubleshooting | 키워드 우선 (3:7) | 빠름 | 중간 |

### 📝 **질문 고도화 (Query Enhancement) - 실제 구현**

#### 1. **Expanded Query (전문 용어 확장)**
```
원본 질문: "브레이크 문제"

🔧 키워드 기반 확장:
- 감지된 키워드: "브레이크" 
- 동의어 사전 매핑: VEHICLE_TERMS['브레이크'] = ['브레이크', '제동', '제동장치', '브레이크 패드', '제동 패드', '브레이크액', '제동액']
- 확장된 쿼리: "브레이크 문제 브레이크 제동 제동장치 브레이크 패드 제동 패드 브레이크액 제동액"

📝 처리 과정: 하이브리드 검색 (벡터 + BM25)으로 검색 범위 확대
```

#### 2. **Multi Query (다중 관점 쿼리 생성)**
```
복잡한 질문: "XC60의 자동 주차 기능은 어떻게 작동하고, 언제 사용할 수 있나요?"

🔧 LLM 기반 3개 관점 확장:
- 관점1 (기술적/전문적): "XC60 자동 주차 시스템 작동 원리와 기술적 구조"
- 관점2 (사용자/실용적): "자동 주차 기능 사용법과 조작 방법"  
- 관점3 (문제해결/안전): "자동 주차 사용 시 주의사항과 안전 가이드"

📝 처리 과정: 각 관점별로 벡터 검색 수행 후 결과 통합
```

#### 3. **실제 선택 로직**
```python
# 1단계: LLM 분석
analysis_result = llm.analyze(query)  # general/specific/troubleshooting + hybrid_semantic/hybrid_balanced/hybrid_keyword

# 2단계: 키워드 규칙 오버라이드 (우선순위 높음)
if any(keyword in query.lower() for keyword in ['교체', '문제', '고장', '이상']):
    search_method = "expanded_query"  # LLM 결과 무시하고 강제 변경
elif len(query) > 20 and '?' in query:
    search_method = "multi_query"     # LLM 결과 무시하고 강제 변경
else:
    search_method = analysis_result   # LLM 분석 결과 사용
```
